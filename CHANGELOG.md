...
### Added / Improved
- **托盘菜单快捷工具条开关**: 系统托盘右键菜单新增“快捷工具条”多选框菜单项 (`checkbox`)，支持从托盘直接切换划词工具条的启用/禁用状态，并自动与设置面板内的开启状态实时同步。
- **划词工具条模块化重构**: 重构主进程划词工具条启停与状态流转逻辑，抽离 `selectionToolbarManager` 统一管理设置面板与托盘菜单的切换机制、macOS 辅助功能权限校验及异常失败回滚逻辑。
