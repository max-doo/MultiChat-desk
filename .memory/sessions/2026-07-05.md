...
- lesson(promoted): 在匹配包含中文等非单词字符（non-word characters）的正则时，需注意默认开启 wordBoundary 时自动添加的 \b（单词边界）会导致匹配失败。此时必须显式指定 wordBoundary: false。
...
- lesson(promoted): will-download 事件是 session 级的，应该只在主进程模块初始化时注册一次，以避免在 IPC 处理器内多次注册导致逻辑冲突
